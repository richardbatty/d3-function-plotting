var Plotter = (function () {
  ...
  function xy() {
    ...
    function chart() {
      var data = types[type](fun, xSpan || [-defaultAxisMax, defaultAxisMax], 700);
      ...
      if (xSpan && ySpan) {
        ...
        y = d3.scale.linear().domain(span.reverse()).range([0, viewportSquareWidth]);
      } else {
        ...
        if (xSpan[0] < -defaultAxisMax || xSpan[1] > defaultAxisMax || ySpan[0] < -defaultAxisMax || ySpan[1] > defaultAxisMax) {
          span = [-defaultAxisMax, defaultAxisMax];
        }
